...
Оцифровка производится через DMA2 Stream0 Channel0.
*/
...
{
  /*Используем оверсемплинг, усреднение значений перед записью в TIM1:CCR1*/
  ...
  TIM1 -> CCR1 = ((ovrsmpl/8) * 28000) / 4096;

  duty =  TIM1 -> CCR1;

  DMA2 -> LIFCR |= DMA_LIFCR_CTCIF0;      //Сброс запроса флага прерывания
  NVIC_ClearPendingIRQ(DMA2_Stream0_IRQn);//Сброс запроса флага прерывания в NVIC
}
