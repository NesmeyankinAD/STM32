...
/*********************************************************************
Принимаем 1 или 0 - зажигаем/тушим светодиод.
Отправляем посылку - массив символов в терминал.
Работа через прерывания на приём и передачу.
*/
...
int main(void)
{
   ...
   */
    ...
    USART1 -> CR1 |= USART_CR1_TCIE;                /* Включение прерывания по окончанию передачи.
                                                      Прерывание активируется, когда была завершена передача байта,
                                                      записанного в регистр DR (transmission complete). В прерывании
                                                      необходимо сбросить запрос флага прерывания.*/  
  ...
  */
  ...
  {   
    ...
    if(TransmitRequired)
    {
        ...
        {
          USART1 -> DR = arr[i];  //Запись байта врегистр DR для передачи
          //Здесь происходит прерывание по окончанию передачи

          for(j = 0;j < TIME/500;j++){} // Задержка
        }
    }
...
//--------Программа обработчика прерывания----------------------//
  ...
  // Как только данные будут записаны в регситр DR
  // начнётся запись данных в регистр TDR и произойдёт передача

 TransmitRequired = true; // Запрос на передачу активирован
...
void USART1_IRQHandler(void)
{
  ...
  else if ((USART1 -> SR & USART_SR_TC) != 0) // Прерывание по окончанию передачи USART_CR_TCIE 
  {
    USART1 -> SR &= ~(USART_SR_TC);           // Сброс флага TC по окончании передачи
  }
