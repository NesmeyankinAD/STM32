...
void ADC_IRQHandler(void)
{
  if (ADC1 -> SR & ADC_SR_EOC)  // Проверяем, что это наш источник
  {
  ...
  */
  ...
  ADC1 -> SR &= ~ADC_SR_EOC;        //Сброс флага запроса прерывания
  NVIC_ClearPendingIRQ(ADC_IRQn); //Сброс статуса в очереди прерываний
  }
}
